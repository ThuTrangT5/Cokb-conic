...
GeometryConicSolver[Deduce_SampleExes] := proc()
	...
	local objType, combs, replace, sample, noParamFacts, hasParamFacts, identifiedFacts, procs, params, doProc, newFacts;
	...
	for s in Sample_Exes do
		...
		for c in combs do
			...
			for p in temp do
				k := Kind_Fact(p);
				for f in Fact_Kinds[k] do
					pt1 := lhs(f)-rhs(f);
					pt2 := lhs(p)-rhs(p);
					result := solve(pt1=pt2, sample[3]);
					if nops(result)>0 then
						thaythe := thaythe union {op(result[1])};
					fi;
				od;
			od;
			...
			procs := sample[6];
			for pr in procs do
				params := convert(subs(thaythe, pr[2]), string);
				params := substring(params, 2..-2);
				doProc := parse(cat(pr[1],"(", params, ")"));
				result := eval(doProc);
				if nops(result) > 0 then
					newFacts := {seq(sample[5][i] = result[i], i=1..nops(sample[5]))};
...
GeometryConicSolver[Determine]:= proc(Goal)
	...
	while not Found and flag do 
		...
		testTime := time();
		Deduce_Funcs(Goal);
		lprint("Buoc 10 : ",time() - testTime);
		if flag then Found:=Test_Goal(Goal,FactSet); next;fi;
		
		#-BUOC 11- 
		# Ap dung Deduce_Eqs de tao ra sk moi bang cach giai pt
		if isPrint= true then lprint("BUOC 11 - vao Deduce_EqsGoal");fi;
		testTime := time();
		Deduce_EqsGoal(Goal); 
		lprint("Buoc 11 : ",time() - testTime);
		if flag then Found:=Test_Goal(Goal,FactSet); next;fi;
		#break; #???              
		if isPrint= true then 
